...
@section('content')
    <div class="col-10 text-center text-white bg-dark shadow-sm rounded-bottom">
        ...
        <p class="lead p-1 m-0" style="font-size: 22px">SERVIÇOS:</p>
    </div>

    <div class="col-12">
        <div class="row d-flex justify-content-end bg-white px-3 py-1">
            <form method="get" action="{{route("servico.create")}}">
                @csrf
        ...
        <ul class="list-group">
            @foreach($servicos as $servico)
                <li class="list-group-item align-items-center d-flex justify-content-between mt-2">ID #{{$servico->id}}
                    |
                    Nome: {{$servico->nome}}
                    <span class="d-flex">
                            <form method="get"
                                  action="{{Route("servico.show",['servico'=>$servico->id])}}">
                                @csrf
                                 <button class="btn btn-outline-dark mr-1"> <i class="fas fa-eye"></i></button>
                            </form>
                        <form method="get" action="{{Route("servico.edit",['servico'=>$servico->id])}}">
                            @csrf
                            <button class="btn btn-outline-dark mr-1"> <i class="far fa-edit"></i> </button>
                        </form>
                        <form method="post" action="{{Route("servico.remover",['id'=>$servico->id])}}"
                              onsubmit="return confirm('Tem certeza que deseja excluir o servico {{$servico->nome}} ?')">
                            @csrf
                            @method('DELETE')
