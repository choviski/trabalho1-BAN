...
@section('content')
    ...
    <div class="row col-12 d-flex justify-content-center mt-2 ">
        <form class="col-12 mt-2" action="{{Route('servico.update',['servico'=> $servico->id])}}" method="post">
            @csrf
            @method('PUT')
            <div class="form-group bg-light p-2 rounded">
                ...
                <input type="number" step="0.01" class="form-control" id="rua" placeholder="Insira o valor" name="valor"
                       value="{{$servico->valor}}" required>

                <label for="bairro">Empregado:</label>
                <select class="form-control" id="tipo" name="id_empregado">
                    @if($servico->empregado)
                        <option id="op1" value="{{$servico->empregado->id}}"> {{$servico->nome }} </option>
                    @endif
                    @foreach($empregados as $empregado)
                        <option id="op1" value="{{$empregado->id}}"> {{$servico->nome }} </option>
                    @endforeach
                </select>
