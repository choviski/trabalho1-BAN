...
@section('content')
    <div class="col-10 text-center text-white bg-dark shadow-sm rounded-bottom">
        ...
        <p class="lead p-1 m-0" style="font-size: 22px">EMPREGADOS:</p>
    </div>

    <div class="col-12">
        <div class="row d-flex justify-content-end bg-white px-3 py-1">
            <form method="get" action="{{route("empregado.create")}}">
                @csrf
        ...
        <ul class="list-group">
            @foreach($empregados as $empregado)
                <li class="list-group-item align-items-center d-flex justify-content-between mt-2">ID #{{$empregado->id}}
                    |
                    Nome: {{$empregado->nome}}
                    <span class="d-flex">
                            <form method="get"
                                  action="{{Route("empregado.show",['empregado'=>$empregado->id])}}">
                                @csrf
                                 <button class="btn btn-outline-dark mr-1"> <i class="fas fa-eye"></i></button>
                            </form>
                        <form method="get" action="{{Route("empregado.edit",['empregado'=>$empregado->id])}}">
                            @csrf
                            <button class="btn btn-outline-dark mr-1"> <i class="far fa-edit"></i> </button>
                        </form>
                        <form method="post" action="{{Route("empregado.remover",['id'=>$empregado->id])}}"
                              onsubmit="return confirm('Tem certeza que deseja excluir o empregado {{$empregado->nome}} ?')">
                            @csrf
                            @method('DELETE')
